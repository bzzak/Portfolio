...
float x_keyboard = 20, x_velocity_keyboard = 0, y_keyboard = SCREEN_HEIGHT / 2 - 10, y_velocity_keyboard = 0, x_velocity_target = 0, y_velocity_target = 0;
	...
	else
	{
		...
		else
		{
			...
			while (!quit)
			{
				...
				SDL_Rect fillAnotherRect = { x_keyboard, y_keyboard, PLAYER1_SIZE, PLAYER1_SIZE };
...
{	
	for (int i = 0; i < data.size(); i++)
	{
		...
		for (int j = 0; j < row.size(); j++) {
			area = { SCREEN_WIDTH / 10 * j, SCREEN_WIDTH / 10 * i, SCREEN_WIDTH / 10, SCREEN_WIDTH / 10 };
			if (row[j] == '/') SDL_RenderCopy(renderer, wallTexture, NULL, &area);
			else if (row[j] == '*') SDL_RenderCopy(renderer, floorTexture, NULL, &area);
			else if (row[j] == '$') SDL_RenderCopy(renderer, doorTexture, NULL, &area);
			else if (row[j] == '+') SDL_RenderCopy(renderer, crateTexture, NULL, &area);
		}
	}
}
